...
ResultSet rs4=null;
ResultSet rs5=null;
ArrayList<Admins> a=new ArrayList<Admins>();
ArrayList<Distributor> d=new ArrayList<Distributor>();	
ArrayList<Customer> c=new ArrayList<Customer>();
ArrayList<Bill_Reciept> b=new ArrayList<Bill_Reciept>();
ArrayList<Staff> s=new ArrayList<Staff>();
	try{
		Class.forName(driver);
		con = DriverManager.getConnection(url, username, pwd);
		ps1 = con.prepareStatement("SELECT A_id from Admins");	
		ps2 = con.prepareStatement("SELECT D_id from Distributor");
		ps3 = con.prepareStatement("SELECT C_id from Customer");
		ps4 = con.prepareStatement("SELECT Bill_id from Bill_Reciept");
		ps5 = con.prepareStatement("SELECT Staff_id from Staff");
	//	ps.setString(1, Item_id);
		...
		while(rs1.next()){
			...
			ad.setA_id(rs1.getString("A_id"));
			a.add(ad);
		}
		while(rs2.next()){
			Distributor di=new Distributor();
			di.setD_id(rs2.getString("D_id"));
			d.add(di);
		}
		while(rs3.next()){
			Customer cu=new Customer();
			cu.setC_id(rs3.getString("C_id"));
			c.add(cu);
		}
		while(rs4.next()){
			Bill_Reciept bi=new Bill_Reciept();
			bi.setBill_id(rs4.getString("Bill_id"));
			b.add(bi);
		}
		while(rs5.next()){
			Staff st=new Staff();
			st.setStaff_id(rs5.getString("Staff_id"));
			s.add(st);
		}
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                	    <ul class="nav navbar-nav">
                             <li class="tab"><a href="Create.jsp">CREATE</a></li>
                            <li class="tab active"><a href="Insert.jsp;">INSERT</a></li>
                ...
                <div class="insert" id="insert" style="display:block">
                     ...
                     <div class="list-group" align="left">
			            ...
			            Insert Items
                    		...
                    		<span href="#" class="list-group-item">
                    		Select Distributor Id: <select class="btn btn-primary 	dropdown-toggle"  type="button" style="width:100px;" data-toggle="dropdown" name="D_id" id="D_id" >
                    		<%
                    		if(d!=null)
                    		{
                    		for(Distributor i:d)
                        	{%>
                        	        <option><%=i.getD_id()%></option>
                       		<%}}
                    		else{%>
                    			<option>None</option>
                    			<%
                    		...
                    		<span href="#" class="list-group-item">
                    		Select Admin Id: <select class="btn btn-primary 	dropdown-toggle"  type="button" style="width:100px;" data-toggle="dropdown" name="A_id" id="A_id" >
                    		<%
                    		if(a!=null)
                    		{
                    		for(Admins i:a)
                        	{%>
                        	        <option><%=i.getA_id()%></option>
                       		<%}}
                    		else{%>
                    			<option>None</option>
                    			<%
			            ...
			            Insert Orders
                    		...
                    		<span href="#" class="list-group-item">
                    		Select Customer Id: <select class="btn btn-primary 	dropdown-toggle"  type="button" style="width:100px;" data-toggle="dropdown" name="C_id" id="C_id" >
                    		<%
                    		if(c!=null)
                    		{
                    		for(Customer i:c)
                        	{%>
                        	        <option><%=i.getC_id()%></option>
                       		<%}}
                    		else{%>
                    			<option>None</option>
                    			<%
                    		...
                    		<span href="#" class="list-group-item">
                    		Select Staff Id: <select class="btn btn-primary 	dropdown-toggle"  type="button" style="width:100px;" data-toggle="dropdown" name="Staff_id" id="Staff_id" >
                    		<%
                    		if(s!=null)
                    		{
                    		for(Staff i:s)
                        	{%>
                        	        <option><%=i.getStaff_id()%></option>
                       		<%}}
                    		else{%>
                    			<option>None</option>
                    			<%
                    		...
                    		<span href="#" class="list-group-item">
                    		Select Bill Id: <select class="btn btn-primary 	dropdown-toggle"  type="button" style="width:100px;" data-toggle="dropdown" name="Bill_id" id="Bill_id" >
                    		<%
                    		if(b!=null)
                    		{
                    		for(Bill_Reciept i:b)
                        	{%>
                        	        <option><%=i.getBill_id()%></option>
                       		<%}}
                    		else{%>
                    			<option>None</option>
                    			<%
